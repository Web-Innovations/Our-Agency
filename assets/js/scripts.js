...
// Function to add the "show" class with a delay for each element
function dropInText() {
    textElements.forEach((textElement, index) => {
        setTimeout(() => {
            textElement.classList.add('show');
        }, index * 500); // Delay each text drop by 500ms
    });
}

// Call the dropInText function when the page loads
window.addEventListener('load', dropInText);

// Add an event listener to track mouse movement over the document
document.addEventListener('mousemove', (e) => {
    // Loop through each text element
    textElements.forEach(textElement => {
        // Get the bounding rectangle of the text element
        const rect = textElement.getBoundingClientRect();
        
        // Get the current mouse position (x, y) within the viewport
        ...
        // Calculate the distance between the mouse and the center of the text element
        const distanceX = textCenterX - mouseX;
        const distanceY = textCenterY - mouseY;

        // Calculate the Euclidean distance between the mouse and the text center
        ...
        if (distance < maxRepelDistance) {
            // Calculate the repel factor based on the distance
            const repelFactor = (1 / distance) * repelStrength;

            // Calculate the offset for the text position based on the repel factor
            const offsetX = (distanceX / distance) * repelFactor;
            const offsetY = (distanceY / distance) * repelFactor;

            // Apply the translation (movement) to the text element
            textElement.style.transform = `translate(${offsetX}px, ${offsetY}px)`;
        } else {
            // If the mouse is outside the maximum repel distance,
